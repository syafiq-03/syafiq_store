                ...
                <article class="article article-style-c"> 
                    ...
                    <div class="article-details"> 
                        ...
                        <div class="article-title"> 
                            <h2><a href="#">Harga: {{ number_format($product->price, 0, ',', '.') }} Points</a></h2> 
                        </div> 

                        <!-- Menambahkan Harga Diskon jika ada Flash Sale -->
                        @if ($product->flashSales->isNotEmpty())
                            @foreach ($product->flashSales as $flashSale)
                                <div class="article-title">
                                    <h2>
                                        <a href="#">Harga Diskon: 
                                            <strong>{{ number_format($flashSale->pivot->discount_price, 0, ',', '.') }} %</strong>
                                        </a>
                                    </h2>
                                </div>
                            @endforeach
                        @else
                            <div class="article-title">
                                <h2><a href="#">Tidak ada diskon saat ini.</a></h2>
                            </div>
                        @endif
